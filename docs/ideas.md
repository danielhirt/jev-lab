...
**Result (2026-09-19).** Built as `packages/codenames`; the gate passed on the first run. On 100 human clues from the SALT-NLP Duet corpus: guess AUC 0.94, target AUC 0.95, top-1 agreement with the human guess 0.64 against a 0.04 base rate, Brier 0.069, repeat std 0.012, 298 ms median through the direct API, $0.00014 per 26-question request. A compact wording without per-word criteria matched the ranking at half the tokens with slightly worse calibration. Full table in the package README. The web app followed the same day: a cooperative board with both modes, a live heatmap while typing a clue, a replay button that reports the largest change across 25 words, and a pipeline strip for Jev-as-spymaster turns; a Jev spymaster turn judges 60 candidates (1,500 word pairs) in about 3 s for under a cent with the fallback proposer.
